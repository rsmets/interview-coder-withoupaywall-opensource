...
export class ProcessingHelper {
  ...
  /**
   * Helper function to determine the correct parameters based on the model
   * GPT-5-mini requires max_completion_tokens instead of max_tokens
   * GPT-5-mini only supports default temperature (1), not custom values like 0.2
   */
  private getModelParameters(model: string): {
    max_tokens?: number;
    ...
    return {
      max_tokens: 4000,
      temperature: 0.2, // Default temperature for other models
    };
  }
  ...
  private async processScreenshotsHelper(
    ...
    try {
      ...
      if (config.apiProvider === "openai") {
        ...
        const extractionResponse =
          await this.openaiClient.chat.completions.create({
            ...
            messages: messages,
            ...this.getModelParameters(config.extractionModel || "gpt-4o"),
          });
      ...
      if (mainWindow) {
        ...
        console.log(
          "🔄 Transitioning from problem extraction to solution generation..."
        );
        const solutionsResult = await this.generateSolutionsHelper(signal);
        if (solutionsResult.success) {
          console.log(
            "✅ Solution generation completed, preparing to send to UI..."
          );
          // Clear any existing extra screenshots before transitioning to solutions view
          this.screenshotHelper.clearExtraScreenshotQueue();

          // Final progress update
          console.log("📤 Sending final progress update to UI (100%)");
          mainWindow.webContents.send("processing-status", {
            message: "Solution generated successfully",
            progress: 100,
          });

          console.log("📡 Broadcasting solution success event to UI");
          mainWindow.webContents.send(
            this.deps.PROCESSING_EVENTS.SOLUTION_SUCCESS,
  ...
  private async generateSolutionsHelper(signal: AbortSignal) {
    console.log("🚀 Starting solution generation process...");
    try {
      ...
      console.log(
        `📋 Problem info loaded: ${problemInfo ? "Available" : "Missing"}`
      );
      console.log(`💻 Target language: ${language}`);
      console.log(
        `⚙️  API provider: ${config.apiProvider || "Not configured"}`
      );
      console.log(`🤖 Model: ${config.solutionModel || "Default"}`);
      ...
      console.log(
        "📤 Sending progress update to UI: Creating optimal solution..."
      );
      if (mainWindow) {
        mainWindow.webContents.send("processing-status", {
...
`;
      ...
      console.log("📝 Generated prompt for AI model");
      console.log(`📊 Prompt length: ${promptText.length} characters`);

      let responseContent;

      if (config.apiProvider === "openai") {
        console.log("🔥 Using OpenAI API for solution generation");
        // OpenAI processing
        if (!this.openaiClient) {
          console.error("❌ OpenAI client not initialized");
          return {
        ...
        console.log(`🤖 Making API call to OpenAI model: ${model}`);
        ...
        const solutionResponse =
          await this.openaiClient.chat.completions.create({
            ...
            ...this.getModelParameters(model),
          });

        console.log("✅ Received response from OpenAI");
        console.log(
          `📊 Response usage: ${JSON.stringify(solutionResponse.usage)}`
        );
        responseContent = solutionResponse.choices[0].message.content;
      } else if (config.apiProvider === "gemini") {
        console.log("🔮 Using Gemini API for solution generation");
        // Gemini processing
        if (!this.geminiApiKey) {
        ...
        try {
          ...
          console.log(`🤖 Making API call to Gemini model: ${model}`);
          ...
          console.log(
            `📊 Response candidates: ${responseData.candidates?.length || 0}`
          );
        ...
        console.log("🧠 Using Anthropic API for solution generation");
        ...
        try {
          ...
          console.log(`🤖 Making API call to Anthropic model: ${model}`);
          console.log("⏳ Waiting for Anthropic response...");
      ...
      console.log(
        `📊 Response length: ${responseContent?.length || 0} characters`
      );
      ...
      console.log(
        `💻 Code extraction: ${
          codeMatch ? "Found code block" : "Using full response as code"
        }`
      );
      console.log(`📏 Extracted code length: ${code?.length || 0} characters`);

      // Extract thoughts, looking for bullet points or numbered lists
      console.log("🧠 Extracting thoughts from response...");
      const thoughtsRegex =
      ...
      if (thoughtsMatch && thoughtsMatch[1]) {
        console.log("✅ Found thoughts section in response");
        ...
        if (bulletPoints) {
          thoughts = bulletPoints
            ...
            .filter(Boolean);
          console.log(`📝 Extracted ${thoughts.length} bullet point thoughts`);
        } else {
          ...
          thoughts = thoughtsMatch[1]
            ...
            .map((line) => line.trim())
            .filter(Boolean);
          console.log(`📝 Extracted ${thoughts.length} line-based thoughts`);
        }
      } else {
        console.log("⚠️  No thoughts section found in response");
      }

      // Extract complexity information
      console.log("⚡ Extracting complexity information...");
      ...
      if (timeMatch && timeMatch[1]) {
        console.log("✅ Found time complexity in response");
      ...
      if (spaceMatch && spaceMatch[1]) {
        console.log("✅ Found space complexity in response");
        ...
        console.log("⚠️  No space complexity found in response, using default");
      }
      ...
      console.log(`   - Thoughts count: ${formattedResponse.thoughts.length}`);
      console.log(
        `   - Time complexity: ${timeComplexity.substring(0, 50)}...`
      );
      console.log(
        `   - Space complexity: ${spaceComplexity.substring(0, 50)}...`
      );

      return { success: true, data: formattedResponse };
    } catch (error: any) {
      console.error("❌ Solution generation failed with error:", error);

      if (axios.isCancel(error)) {
        console.log("🚫 Solution generation was canceled by user");
        return {
          success: false,
          error: "Processing was canceled by the user.",
        };
      ...
      console.error("💥 Unexpected solution generation error:", {
...
4. A clear explanation of the changes needed`,
        ...
        const debugResponse = await this.openaiClient.chat.completions.create({
          ...
          messages: messages,
          ...this.getModelParameters(config.debuggingModel || "gpt-4o"),
        });
